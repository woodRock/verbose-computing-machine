...
score paths = length $ [(x,y) | (x,y,h) <- concat paths, h == 9]

walkPath :: (Int, Int) -> Set.Set (Int, Int) -> [[Int]] -> [[(Int, Int, Int)]]
walkPath c@(x,y) visited heights 
  | isSummit = [[(x,y, 9)]]
  | null nextPaths = []
  | otherwise = case allPaths of
                 [] -> []
                 paths -> filter ((== maxLen) . length) paths
                   where maxLen = maximum $ map length paths
  where 
    isSummit = height == 9
    height = heights !! y !! x
...
path x = concatMap (\s -> walkPath s Set.empty heights) starts
  where 
    heights = map (map digitToInt) x
    starts = findStarts x
    getLongestPaths paths = filter ((== maxLen) . length) paths
      where maxLen = maximum $ map length paths

debug :: [[(Int, Int, Int)]] -> String
debug paths = unlines $ map show paths
